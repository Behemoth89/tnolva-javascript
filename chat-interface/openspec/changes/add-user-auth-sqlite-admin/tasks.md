...
- [x] 1.1 Add runtime deps to `backend/package.json`: `better-sqlite3`,
  `bcrypt`, `express-session`, `better-sqlite3-session-store`,
  `express-rate-limit`, plus matching `@types/*` packages for the typed
  ones.
- [x] 1.2 Extend `backend/src/config.ts` to read `SESSION_SECRET`,
...
- [x] 1.3 Update `backend/.env.example` with the new variables and a comment
  noting that `SESSION_SECRET` must be set in production.

## 2. Backend persistence

- [x] 2.1 Create `backend/src/db.ts` exporting `initDb(databasePath)` and a
  ...
  value).
- [x] 2.2 Add a `users` repository module (`backend/src/auth/usersRepo.ts`)
...
- [x] 4.2 Update `backend/src/index.ts` to call `initDb(config.databasePath)`
  before `app.listen`, logging a one-line confirmation (no secrets, no
  user data).
- [x] 4.3 Add `backend/data` to `.gitignore` (or extend the existing
  backend `.gitignore`) so the SQLite file is not committed.

## 5. Frontend dependencies & API client

- [x] 5.1 Add `react-router-dom` to `frontend/package.json` dependencies.
- [x] 5.2 Create `frontend/src/api/auth.ts` exporting `fetchMe`,
...
- [x] 6.7 Refactor `frontend/src/App.tsx` to wrap the app in
...
- [x] 7.1 Add a named volume `chat-data` in `docker-compose.yml` and mount
  ...
  block.
- [x] 7.2 Add a "Production HTTPS required" note to `README.md` and a
...
- [x] 7.3 Update `backend/.dockerignore` and `frontend/.dockerignore` if
  needed to ensure the new source files are copied in but the local
  `data/` directory is excluded.

## 8. Backend tests

- [x] 8.1 Unit test `passwords.ts` covering: hash differs from plaintext,
...
- [x] 8.2 Unit test the first-user-admin rule in `usersRepo`: with an
  empty DB, `createUser` returns `isAdmin: true`; with one existing user,
  a second `createUser` returns `isAdmin: false`.
- [x] 8.3 Integration tests under `backend/tests/integration/` (Jest +
...
- [x] 8.4 Verify all backend tests pass with `cd backend && npm test`.

## 9. Frontend tests

- [x] 9.1 Component test for `Login`: submitting valid credentials via a
...
- [x] 9.2 Component test for `Register`: mismatched confirm-password
...
- [x] 9.6 Verify all frontend tests pass with `cd frontend && npm run test:run`.
